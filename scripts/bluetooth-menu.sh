...
if ! bt_powered; then
    entries="$HEADER\n$SEPARATOR\n$TOGGLE_ON_ITEM\n$SETTINGS_ITEM"
    selected=$(printf '%b' "$entries" | walker --dmenu --theme tokyonight-compact --placeholder "Bluetooth" --width 260 --height 150)
    case "$selected" in
...
selected=$(printf '%b' "$entries" | walker --dmenu --theme tokyonight-compact --placeholder "Bluetooth" --width 260 --height 260)
